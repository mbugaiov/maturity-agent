...
1. Find the **highest level where routine delivery works without human intervention** on the target environment.
2. If humans still **must** click Accept/Merge/Deploy for every ticket → cap at **L3–L4**.
3. If only **intent + exceptions** are human → **L5′** or **L5**.
4. Report as **primary level** + **direction** (e.g. "L4, reaching L5′ on STG") + **% factory complete** when useful.
