...
> Operating constraints: `.cursor/rules/maturity-engine.mdc` (always-on).
> Procedural detail: skills under `.cursor/skills/`.
...
3. Discover  → maturity-interview: repos, .cursor/, CI, Jira, deploy, example ticket trace
...
- **Evidence before score** — no level claim without a cited artifact (file path, Jira key, CI log, script).
